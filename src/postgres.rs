...
// fn to_job_status(i: i32) -> PostgresJobStatus {
//     match i {
//         1 => PostgresJobStatus::Queued,
//         2 => PostgresJobStatus::Running,
//         _ => panic!("couldn't map to job status"),
//     }
// }
...
impl PostgresQueue {
    ...
    async fn run_worker(&self) {
        loop {
            let jobs = match self.pull(CONCURRENCY as u32).await {
                Ok(jobs) => jobs,
                Err(err) => {
...
impl Queue for PostgresQueue {
    ...
    async fn pull(&self, number_of_jobs: u32) -> Result<Vec<Job>, crate::Error> {
        ...
        let jobs: Vec<PostgresJob> = sqlx::query(query)
